...
app.post("/teacher/signup", (req, res) => {
    ...
    if (req.body.password.length < 6) {
        res.render("homePage", { error: "Password length cannot be less than 6" });
    }
    ...
    req.login(user, (err) => {
        if (err) {
            ...
            classroomCodes.findOne({ username: req.body.username }, (err, obj) => {
                if (err) {
                    console.log(err);
                    res.render("homePage", { error: "Login failed" });
                } else {
                    class_code = obj.classroom_code;
                }
            });

            passport.authenticate("local")(req, res, (err) => {
                if (err) {
                    res.render("homePage", { error: "Problems occured while signing up. Please try again." })
                }
                res.redirect("/hostPage/" + class_code);
            });
        }
    });
});
...
app.get("/hostPage/:classCode", (req, res) => {
    ...
    if (req.isAuthenticated()) {
        ...
        res.render("homePage", { error: "Please login to continue." });
    }
});
...
app.post("/createForm/:class_code", (req, res) => {
    ...
    storage.findOneAndUpdate({ classroom_code: req.params.class_code }, { form: req.body.inputName }, (err) => {
        if (err) {
            console.log(err);
        }
    });
    ...
    storage.findOne({ classroom_code: classCode }, (err, obj) => {
        if (err) {
            ...
            if (obj == null) {
                ...
                res.redirect('/joinPage/' + classCode);
            }
        }
    })
});
    ...
    storage.findOne({ classroom_code: classCode }, (err, obj) => {
        if (err) {
            ...
            if (obj == null) {
                ...
                res.render("joinExam", { classCode: classCode, formData: obj.form });
            }
        }
    });
...
app.post("/checkClassCode", (req, res) => {
    ...
    storage.findOne({ classroom_code: classCode }, (err, obj) => {
        if (err) {
            ...
            res.redirect("/joinExam/" + classCode);
        }
    });
});
